/*
  ...
  Comprehensive migration untuk sistem cashflow, HR, dan Sariroti field sales.
  ...
  - login_history - Track user login activities
  - invite_links - System untuk invite user baru
  - invite_link_usage - Track penggunaan invite links
  ...
  - cashflow_forecasts - Prediksi cashflow
  - transaction_tags - Tags untuk transaksi
  - category_hierarchy - Hierarki kategori
  
  ### Employee & Payroll Management
  - positions - Jabatan karyawan
  - job_positions - Posisi pekerjaan dengan base salary
  - employees - Data karyawan
  - employee_loans - Pinjaman karyawan
  - salary_payments - Pembayaran gaji
  - batch_salary_payments - Batch processing gaji
  - salary_payment_proofs - Bukti pembayaran gaji
  - salary_components - Komponen gaji (income/deduction)
  - employee_policies - Kebijakan karyawan
  ...
  - ai_tips_settings - Pengaturan AI tips
  - admin_dashboard_preferences - Preferensi dashboard
  - sariroti_settings - Pengaturan per karyawan Sariroti
  
  ### Reporting & Logging
  - export_logs - Log export data
  - bulk_operation_logs - Log operasi bulk
  - report_templates - Template laporan
  - report_schedules - Jadwal laporan otomatis
  - export_templates - Template export
  
  ### Sariroti Field Sales
  - stores - Data toko/outlet
  - visit_plans - Rencana kunjungan
  - visit_checkins - Check-in kunjungan toko
  - bread_scans - Scan produk roti
  - sku_items - Master data SKU produk (akan di-seed terpisah)
  
  ### Additional Tables
  - announcements - Pengumuman untuk karyawan
  - holidays - Data hari libur
  - backup_schedules - Jadwal backup
  - integrations - Integrasi dengan sistem lain
  
  ## Security
  - RLS enabled pada semua tabel
  - Policies disesuaikan dengan role-based access
  - Data isolation per user_id
  
  ## Notes
  1. Default superadmin akan dibuat: admin@admin.com / admin123
  2. Semua tabel menggunakan uuid sebagai primary key
  3. Foreign key constraints untuk data integrity
  4. Indexes untuk performa query
*/

-- Custom users table (replaces Supabase auth.users for custom auth)
...
CREATE POLICY "Users can read own data"
  ON users FOR SELECT
  TO authenticated
  USING (id = (SELECT user_id FROM user_profiles WHERE user_id = auth.uid()));
...
-- Add employee_id foreign key to user_profiles
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'fk_user_profiles_employee_id'
  ) THEN
...
CREATE POLICY "Users can manage own employee loans"
...
CREATE POLICY "Users can read invite link usage"
...
CREATE POLICY "Users can manage own cashflow forecasts"
...
CREATE POLICY "Users can manage own batch salary payments"
...
CREATE POLICY "Users can manage salary payment proofs"
  ...
  TO authenticated
  USING (EXISTS (
    SELECT 1 FROM batch_salary_payments WHERE id = batch_payment_id AND user_id = auth.uid()
  ));
...
CREATE POLICY "Users can read own bulk operation logs"
...
CREATE POLICY "Sariroti staff can manage stores"
  ON stores FOR ALL
  TO authenticated
  USING (EXISTS (
    SELECT 1 FROM user_profiles WHERE user_id = auth.uid() AND role IN ('admin_sariroti', 'karyawan_sariroti')
  ));
...
CREATE POLICY "Users can manage own visit plans"
  ON visit_plans FOR ALL
  TO authenticated
  USING (user_id = auth.uid());

CREATE POLICY "Admins can view all visit plans"
...
CREATE POLICY "Users can manage own visit checkins"
  ON visit_checkins FOR ALL
  TO authenticated
  USING (user_id = auth.uid());

CREATE POLICY "Admins can view all visit checkins"
...
ALTER TABLE bread_scans ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can manage own bread scans"
  ON bread_scans FOR ALL
  TO authenticated
  USING (user_id = auth.uid());

CREATE POLICY "Admins can view all bread scans"
...
ALTER TABLE sariroti_settings ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can read own sariroti settings"
  ON sariroti_settings FOR SELECT
  TO authenticated
  USING (user_profile_id IN (SELECT id FROM user_profiles WHERE user_id = auth.uid()));

CREATE POLICY "Admins can manage all sariroti settings"
  ON sariroti_settings FOR ALL
  TO authenticated
  USING (EXISTS (
    SELECT 1 FROM user_profiles WHERE user_id = auth.uid() AND role IN ('admin_sariroti', 'superadmin')
  ));

-- SKU Items table (master data SKU produk)
