            ...
            <table className="w-full">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Dept / Jabatan</th>
                    ...
                    <tr className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="text-sm font-medium text-gray-900">{up.full_name || '—'}</p>
                            <p className="text-xs text-gray-400">{up.email}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-4 py-3">
                        <span className="text-sm text-gray-700 font-mono">{up.nik || '—'}</span>
                      </td>
                      <td className="px-4 py-3">
                        <p className="text-sm text-gray-800">{up.department || '—'}</p>
                        <p className="text-xs text-gray-400">{up.job_title || ''}</p>
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3 text-sm text-gray-600">{up.phone || '—'}</td>
                      <td className="px-4 py-3">
                        <span className={`px-2 py-1 text-xs font-medium rounded-full ${
                          up.status === 'inactive' ? 'bg-red-100 text-red-700' : 'bg-emerald-100 text-emerald-700'
                        }`}>
                      ...
                      <td className="px-4 py-3 text-right">
                        <div className="flex items-center justify-end gap-1">
                          <button onClick={() => setExpandedUser(expandedUser === up.id ? null : up.id)}
                            className="p-1.5 text-gray-400 hover:text-gray-600 rounded" title="Detail">
                            {expandedUser === up.id ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
                          </button>
                          {up.email !== 'admin@admin.com' && (
                            <>
                              <button onClick={() => openEdit(up)} className="p-1.5 text-blue-500 hover:text-blue-700 rounded" title="Edit">
                                <Edit2 className="w-4 h-4" />
                              </button>
                              <button onClick={() => setDeleteConfirm(up.user_id)} className="p-1.5 text-red-400 hover:text-red-600 rounded" title="Hapus">
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                            <div className="flex items-start gap-2">
                              ...
                              <div>
                                ...
                                <p className="font-medium text-gray-800">{up.nik || '—'}</p>
                              </div>
                            </div>
                            <div className="flex items-start gap-2">
                              ...
                              <div>
                                ...
                                <p className="font-medium text-gray-800">{up.phone || '—'}</p>
                              </div>
                            </div>
                            <div className="flex items-start gap-2">
                              ...
                              <div>
                                ...
                                <p className="font-medium text-gray-800">{up.department || '—'}</p>
                              </div>
                            </div>
                            <div className="flex items-start gap-2">
                              ...
                              <div>
                                ...
                                <p className="font-medium text-gray-800">{up.job_title || '—'}</p>
                              </div>
                            </div>
                            <div className="flex items-start gap-2">
                              <Calendar className="w-4 h-4 text-gray-400 mt-0.5 flex-shrink-0" />
                              <div>
                                <p className="text-xs text-gray-400">Tgl Lahir</p>
                            ...
                            <div className="flex items-start gap-2">
                              <Calendar className="w-4 h-4 text-gray-400 mt-0.5 flex-shrink-0" />
                              <div>
                                <p className="text-xs text-gray-400">Tgl Bergabung</p>
                                <p className="font-medium text-gray-800">
                            ...
                            <div className="flex items-start gap-2 col-span-2">
                              ...
                              <div>
                                ...
                                <p className="font-medium text-gray-800">{up.address || '—'}</p>
                              </div>
                            </div>
                            <div>
                              <p className="text-xs text-gray-400">Jenis Kelamin</p>
                              <p className="font-medium text-gray-800">{up.gender || '—'}</p>
                            </div>
                            <div>
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-sm w-full p-6">
            ...
            <p className="text-gray-600 mb-6">Apakah Anda yakin ingin menghapus user ini? Tindakan ini tidak bisa dibatalkan.</p>
          ...
          <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full p-6">
            ...
            {generatedLinks.length === 0 ? (
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Role untuk User Baru</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Jumlah Link (1-20)</label>
                  <input type="number" min="1" max="20" value={inviteCount}
                    onChange={e => setInviteCount(parseInt(e.target.value) || 1)}
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" />
                  <p className="text-xs text-gray-500 mt-1">Setiap link hanya bisa digunakan 1 kali</p>
                </div>
              ...
              <div className="space-y-4">
                <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-4">
                  <p className="text-emerald-800 font-medium">{generatedLinks.length} invite link berhasil dibuat!</p>
                  <p className="text-sm text-emerald-700 mt-1">Role: <span className="font-semibold">{getRoleName(inviteRole)}</span></p>
                </div>
                <div className="max-h-80 overflow-y-auto space-y-2">
                  {generatedLinks.map((link, index) => (
                    <div key={link.id} className="p-3 bg-gray-50 rounded-lg border border-gray-200 flex items-center justify-between gap-3">
                      <div className="flex-1 min-w-0">
                        <p className="text-xs text-gray-500 mb-1">Link #{index + 1}</p>
                        <code className="text-xs text-gray-700 break-all">{window.location.origin}/invite/{link.invite_token}</code>
                      </div>
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-2 flex-wrap">
                          ...
                          <span className={`px-2 py-1 text-xs font-medium rounded-full ${link.is_active ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'}`}>
                            {link.is_active ? 'Active' : 'Inactive'}
                          </span>
                          <span className="text-xs text-gray-500">{link.current_uses}/{link.max_uses} digunakan</span>
                        </div>
                        <code className="text-xs text-gray-700 break-all block">{window.location.origin}/invite/{link.invite_token}</code>
                        <p className="text-xs text-gray-500 mt-1">Dibuat: {new Date(link.created_at).toLocaleString('id-ID')}</p>
                      </div>
                      <div className="flex gap-2 flex-shrink-0">
                        ...
                        {link.current_uses === 0 && (
                          <button onClick={() => toggleInviteActive(link.id, link.is_active)}
                            className={`px-3 py-2 rounded-lg text-xs font-medium transition-colors ${link.is_active ? 'bg-red-100 text-red-600 hover:bg-red-200' : 'bg-emerald-100 text-emerald-600 hover:bg-emerald-200'}`}>
